...
**Nombre del Archivo Entregable:** `S02_LP2_Equipo05_CalisayaHelio.pdf`
...
* **Rol o Aporte Realizado:** Desarrollador Backend & Integración de Controladores REST, DTOs, Mappers, Validaciones, Pruebas `@WebMvcTest` y Conexión Oracle 21c XE.
...
2. **¿Cuál es el aprendizaje más importante que te llevas de la clase de hoy?**
   * El desacoplamiento del controlador REST mediante Mappers dedicados, DTOs con validaciones explícitas (`@Valid`) y el manejo centralizado de excepciones con `@RestControllerAdvice`.
